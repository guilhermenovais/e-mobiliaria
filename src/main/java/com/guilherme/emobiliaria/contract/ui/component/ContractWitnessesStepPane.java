...
public class ContractWitnessesStepPane extends VBox {
  ...
  public ContractWitnessesStepPane(ResourceBundle bundle) {
    this.bundle = bundle;
  ...
  private void buildLayout() {
    Label hint = new Label(bundle.getString("contract.wizard.witnesses.hint"));
    hint.getStyleClass().add("wizard-hint-label");

    Label availableHeader = new Label(bundle.getString("contract.wizard.witnesses.available"));
    availableHeader.getStyleClass().add("form-label");
    searchField.setPromptText(bundle.getString("contract.wizard.witnesses.search.prompt"));
    ...
    Label selectedHeader = new Label(bundle.getString("contract.wizard.witnesses.selected"));
  ...
  public void populate(List<Person> witnesses) {
    selectedList.setAll(witnesses);
    List<Person> remaining = new ArrayList<>(availableList);
    remaining.removeIf(p -> witnesses.stream().anyMatch(w -> w.getId().equals(p.getId())));
    availableList.setAll(remaining);
    applyFilter(searchField.getText());
  }

  public List<Person> getSelectedWitnesses() {
