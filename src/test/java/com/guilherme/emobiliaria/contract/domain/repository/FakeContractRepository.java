...
public class FakeContractRepository extends FakeImplementation implements ContractRepository {
  ...
  public Optional<Contract> findById(Long id) {
    maybeFail();
    return Optional.ofNullable(store.get(id)).map(c -> {
      resolveStatuses(List.of(c));
      return c;
    });
  }
  ...
  public PagedResult<Contract> search(String query, PaginationInput pagination,
    ...
    List<Contract> filtered = store.values().stream().filter(c -> {
      ...
      boolean matchesTenant = c.getTenants() != null && c.getTenants().stream().anyMatch(t -> {
        if (t instanceof PhysicalPerson pp)
          return pp.getName().toLowerCase().contains(lower);
        if (t instanceof JuridicalPerson jp)
          return jp.getCorporateName().toLowerCase().contains(lower);
        return false;
      });
  ...
  private void resolveStatuses(List<Contract> contracts) {
    Comparator<Contract> latestFirst =
        Comparator.comparing(Contract::getStartDate).thenComparingLong(Contract::getId);
    Map<Long, Long> latestIdPerProperty = store.values().stream()
        .filter(c -> c.getProperty() != null && c.getProperty().getId() != null).collect(
            Collectors.groupingBy(c -> c.getProperty().getId(),
                Collectors.collectingAndThen(Collectors.maxBy(latestFirst),
                    opt -> opt.map(Contract::getId).orElse(0L))));
    contracts.forEach(c -> {
      if (c.getProperty() != null && c.getProperty().getId() != null) {
